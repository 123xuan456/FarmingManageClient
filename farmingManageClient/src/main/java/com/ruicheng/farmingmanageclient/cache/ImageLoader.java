...
import com.ruicheng.farmingmanageclient.cache.ImageSizeUtil.ImageSize;
...
public class ImageLoader
{
        ...
        return new Runnable()
        {
            ...
            public void run()
            {
                ...
                if (isFromNet)
                {
                    ...
                    {
                        if (isDiskCacheEnable)// 棿Ջ是否弿Я硬盘缓存
                        {
    ...
    public String bytes2hex02(byte[] bytes)
    {
        ...
        for (byte b : bytes)
        {
            // 将每个字节与0xFF进行与运算，然后转化丿0进制，然后急ʩ于Integer再转化为16进制
            tmp = Integer.toHexString(0xFF & b);
            if (tmp.length() == 1)// 每个字节8为，转为16进制标志＿丿6进制使
            {
                tmp = "0" + tmp;
            }
            sb.append(tmp);
